...
| M15–M20 | WBS, Meeting, Risk, Inspection, Closeout | COMPLETE | Thin vertical slice bundling the post-proposal work-stream tables (wbs_tasks + wbs_deliverables, meetings + meeting_minutes, risks, inspections, closeouts) with project/tenant-admin RLS, immutable FK to the Requirement candidate, and audit-friendly status fields. Single read-only `/projects/[projectId]/runs/[runId]/workspace` page surfaces all five domains from the run-scoped lineage. DB reset + RLS 7/7 pass. E2E for these thin pages deferred to M22 prod build (workspace page itself proves runtime compile + data binding in dev). |
| M21 | Hosted Supabase | BLOCKED (H4 + H5 required) | M21 requires user-provided Supabase access token (H4) and Production Region choice (H5). Cannot proceed without user inputs. |
| M22 | Cloudflare Production (build probe OK) | IN_PROGRESS (blocked H3) | `scripts/verify-workers-preview.ps1` ran a Linux `node:24.19.0-bookworm-slim` container that successfully produced the production build and served HTTP 200 with the GOV Project OS body — production-build integration confirmed at the container level. Cloudflare account + wrangler login (H3) + production Supabase link (H4) + production region (H5) are required to deploy. M13–M20 E2E re-validation is queued for prod. |
| M23 | Final System Verification | IN_PROGRESS (depends M22) | M23 will run after M22 prod build. Pending that, the dev-side verification (M12) is fresh and the M22 build probe (above) confirms production-bound integration. External dependencies (Cloudflare account, production Supabase project, production region) are user decisions (H3/H4/H5). |
