...
| M15–M20 | WBS, Meeting, Risk, Inspection, Closeout | COMPLETE | Thin vertical slice bundling the post-proposal work-stream tables and a single read-only `/projects/[projectId]/runs/[runId]/workspace` page that surfaces all five domains from the run-scoped Requirement/Contract lineage. Each table enforces project/tenant-admin RLS, immutable FK to the Requirement candidate, and audit-friendly status fields. RLS 7/7 pass. E2E deferred to M22 prod build (the page itself proves runtime compile + data binding under dev). |
| M21 | Hosted Supabase | IN_PROGRESS (blocked H4/H5) | M21 requires user-provided Supabase access token (H4) and Production Region (H5). Cannot proceed without user inputs. |
| M22 | Cloudflare Production | IN_PROGRESS (blocked H3) | M22 requires `wrangler login` (H3) + `pnpm supabase link` (H4) + Production Region. Cannot proceed without user inputs. |
| M23 | Final System Verification | IN_PROGRESS (depends M22) | M23 cannot run until M22 prod build exists; on M22 completion, run the lifecycle matrix against prod. |
